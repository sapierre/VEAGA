...
export function AreaChart() {
  ...
  return (
    <Card className="w-full">
            ...
            <>
              <StackedArea
                ...
                areaOptions={({ rowIndex, lowestY, highestY }) => {
                  const [h, s, l] =
                    palette[
                      Object.values(chartConfig)[rowIndex]?.color ?? "chart-1"
                    ];
                  const colors = [
                    `hsl(${h}, ${s * 100}%, ${l * 100}%)`,
                    `hsla(${h}, ${s * 100}%, ${l * 100}%, 0.3)`,
                  ];
                  switch (rowIndex) {
                    case 0:
                      return {
      ...
      <CardFooter className="mx-auto flex-row items-center gap-4">
        {Object.values(chartConfig).map((config) => (
          <View key={config.color} className="flex-row items-center gap-2">
            <View className={`size-3 rounded-sm bg-${config.color}`} />
